...
const Storage = (() => {
  ...
  const Usuarios = {
    ...
    async create({ nombre, usuario, email, pais, rol, password }) {
      ...
      if (email) {
        await sb.from('perfiles').update({ email }).eq('id', data.user.id);
      }
  ...
  const CatalogoRep = {
    async buscar(codigo) {
      ...
      return {
        ...
        telefonoGerenteZona: data.telefono_gerente_zona
      };
    },
    async getAll() {
      ...
      return (data || []).map(d => ({
        ...
        telefonoGerenteZona: d.telefono_gerente_zona, actualizado: d.actualizado_en
      }));
    },
    async upsertMuchos(filas) {
      // filas: [{ codigoRep, zona, gerenteZona, telefonoRep, sector, pdActual, saldoLocal, nombreRep, telefonoGerenteZona }, ...]
      const payload = filas.filter(f => f.codigoRep).map(f => ({
        ...
        telefono_gerente_zona: f.telefonoGerenteZona || null, actualizado_en: new Date().toISOString()
      }));
